...
beta_c = 1e-6 #1e-4 # 4e-4   # cloud peak bs coefficient
...
Tetalon = O2_Filter.spectrum(np.concatenate((np.arange(-50e9,50e9,10e6)+lp.c/wavelength_list[4],np.arange(-50e9,50e9,10e6)+lp.c/wavelength_list[5])),InWavelength=False,aoi=Etalon_angle,transmit=True)
plt.figure(); 
...
plt.plot(1e9*lp.c/(sim_nu+lp.c/wavelength_list[5]),laser_spec[name_list[5]]/laser_spec[name_list[5]].max(),label=name_list[5]+' Laser')
#plt.stem(1e9*np.array([wavelength_list[4],wavelength_list[5]]),np.ones(2),label='Laser')
plt.plot(1e9*lp.c/(np.concatenate((np.arange(-50e9,50e9,10e6)+lp.c/wavelength_list[4],np.arange(-50e9,50e9,10e6)+lp.c/wavelength_list[5]))),Tetalon,label='Etalon'); 
