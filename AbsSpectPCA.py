...
#if 'WV' in name_list[sim_i]:
#    start_time = timeit.default_timer()
#    ext_o2 = td.ExtinctionFromHITRAN(lp.c/wavelength_list[sim_i]+sim_nu,Tm.flatten(),Pm.flatten(),(species_mass_list[sim_i]*1e-3)/lp.N_A,nuLim=spec_range[i],freqnorm=True,filename=spec_file[sim_i]).T
#    elapsed = timeit.default_timer() - start_time
#else:
#    start_time = timeit.default_timer()
#    ext_o2 = td.ExtinctionFromHITRAN(lp.c/wavelength_list[sim_i]+sim_nu,Tm.flatten(),Pm.flatten(),(td.mO2*1e-3)/lp.N_A,nuLim=np.array([lp.c/770e-9,lp.c/768e-9]),freqnorm=True,filename=spec_file[sim_i]).T
#    elapsed = timeit.default_timer() - start_time

## build HITRAN spectrum for all T and P combinations
#for ai in range(len(sim_P)):
#    ext_o2[:,ai*sim_T.size:(ai+1)*sim_T.size] = td.ExtinctionFromHITRAN(lp.c/wavelength_list[1]+sim_nu,sim_T,sim_P[ai]*np.ones(sim_T.size),(td.mO2*1e-3)/lp.N_A,nuLim=np.array([lp.c/770e-9,lp.c/768e-9]),freqnorm=True,filename=o2_spec_file).T

# perform SVD    
spect_mean = np.mean(ext_o2,axis=1)
u,s,v = np.linalg.svd(ext_o2-np.mean(ext_o2,axis=1)[:,np.newaxis])

# truncate princpal components to the first Npca
uPCA = u[:,:Npca]

# calculate PCA weights for each T and P combination
w = np.matrix(uPCA.T)*np.matrix(ext_o2-np.mean(ext_o2,axis=1)[:,np.newaxis])
...
#Pmean
#Pstd
T_lims = np.array([sim_T[-1],sim_T[0]])
P_lims = np.array([sim_P[-1],sim_P[0]])
#Npoly
#Npca
# run_string
# report_string
...
maxError = np.nanmax(np.abs(ext_o2-ext_pca)/ext_o2,axis=0)
RMSerror = np.sqrt(np.sum((ext_o2-ext_pca)**2,axis=0))/np.sum(ext_o2,axis=0)
maxError_poly = np.nanmax(np.abs(ext_o2-ext_poly)/ext_o2,axis=0)
RMSerror_poly = np.sqrt(np.sum((ext_o2-ext_poly)**2,axis=0))/np.sum(ext_o2,axis=0)
diff_pca_poly = np.max(np.abs(ext_pca-ext_poly)/ext_o2,axis=0)
diff_pca_poly_RMS  = np.sqrt(np.sum((ext_pca-ext_poly)**2,axis=0))/np.sum(ext_o2,axis=0)
diff_pca_poly_RMS_total  = np.sqrt(np.sum((ext_pca-ext_poly)**2))/np.sum(ext_o2)
RMSError_poly_total = np.sqrt(np.sum((ext_o2-ext_poly)**2))/np.sum(ext_o2)
RMSerror_total = np.sqrt(np.sum((ext_o2-ext_pca)**2))/np.sum(ext_o2)
...
for irun in range(i_test_array.size):
    ...
    TPmat_test = ((Tm.flatten()-Tmean)/Tstd)[i_test,np.newaxis]**pT[np.newaxis,:]*((Pm.flatten()-Pmean)/Pstd)[i_test,np.newaxis]**pP[np.newaxis,:]
    spec_poly = np.array(M*TPmat_test.T+Mavg).flatten()
    plt.figure();
    plt.plot(sim_nu*1e-9,ext_o2[:,i_test])
