...
def TDProfiles(Prof,x,Trx,rb_spec,abs_spec,dr,inu0,bsrMult,base_T,base_P,r0):
    ...
    HSRL_mol = HSRLProfile(x['T'],x['B'],x['Phi'],rb_spec['HSRL'],Trx['HSRL Mol'],inu0['HSRL'],xK[0],base_T,base_P)
...
def HSRLProfile(x,rb_spec,Trx,inu0,K,base_T,base_P):
...
def HSRLProfile(x,Const,dt=1,dR=37.5,return_params=False,params={},n_conv=0,scale={'xB':1,'xPhi':1,'xT':1}):
    """
    ...
    Const - constant terms used throughout the routine for each channel.  It expects
        a dict entry for 'HSRL Mol' and 'HSRL Comb' each containing
            ...
            'Ta' - receiver aerosol transmission
            'Tm' - receiver molecular transmission
        'HSRL'
            'rb_spec' - Rayleigh-Brillouin parameters for the HSRL entry
    """
    ...
    try:
        ...
        T = scale['xT']*x['xT'] # np.exp(scale['xN']*x['xN'])  or scale['xN']*x['xN']
        phi = np.exp(scale['xPhi']*x['xPhi'])
        B = np.exp(scale['xB']*x['xB'])
    ...
    betaM = calc_pca_spectrum(Const['HSRL']['rb_spec'],T,Const['base_T'],Const['base_P']) 
    forward_profs = {}
    for var in Const.keys():
        # get index into the gain and deadtime arrays
        if 'HSRL Mol' in var:
            iConst = 0
        elif 'HSRL Comb' in var:
            iConst = 1
            
        if var != 'kconv':    
#            forward_profs[var] = np.exp(x['xG'][iConst])*Const[var]['mult']*phi*np.exp(-2*dR*np.cumsum(nWV*Const[var]['sigma'],axis=1))+Const[var]['bg']
            forward_profs[var] = np.exp(x['xG'][iConst])*Const[var]['mult']*phi*(Const[var]['Ta']*(1-1/B)+1/B*np.sum(Const[var]['Tm'][:,np.newaxis]*betaM,axis=0))
            if kconv.size > 1:
...
#                forward_profs[var] = forward_profs[var]*dt[:,np.newaxis]/(dt[:,np.newaxis]+forward_profs[var]*np.exp(x['xDT'][iConst]))
